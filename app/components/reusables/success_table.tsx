...
export default function Success_table({
  ...
  const renderMessage = (msg?: string | MessageItem, key?: string) => {
    ...
    return (
      <span className="d-inline-flex justify-center align-items-center gap-2">
        {msg.text}

        {msg.copy && (
          <button
            onClick={() => handleCopy(key!, msg.copy!)}
            className="btn btn-sm p-0 border-0 bg-transparent ml-1"
        ...
        <table className="w-full border-collapse text-[14px] text-(--text-1)">
          <tbody>
            ...
            {col3 && message3 && (
              <tr
                className={
                  col4 && message4 ? "border-b border-b-(--grey-1)" : ""
                }
              >
